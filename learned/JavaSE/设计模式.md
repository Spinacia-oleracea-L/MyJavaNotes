...
1. 装饰模式是⼀种结构型设计模式， 允许你通过将对象放⼊包含⾏为的特殊封装对象中来为原对象绑定新的⾏为。
...
1. 过滤器模式（Filter Pattern）或标准模式（Criteria Pattern）是一种设计模式，这种模式允许开发人员使用不同的标准来过滤一组对象，通过逻辑运算以解耦的方式把它们连接起来。
2. 这种类型的设计模式属于结构型模式，它结合多个标准来获得单一标准。
...
1. 中介者模式Mediator属于⾏为模式， 能让你减少对象之间混乱⽆序的依赖关系。 该模式会限制对象之间的直接交互， 迫使它们通过⼀个中介者对象进⾏合作。
...
1. 模板⽅法模式（Template Method）是⼀种⾏为设计模式， 它在超类中定义了⼀个算法的框架， 允许⼦类在不修改结构的情况下重写算法的特定步骤。
2. 定义一个操作中的算法的骨架，而将一些步骤延迟到子类中。模板方法使得子类可以不改变一个算法的结构即可重定义该算法的某些特定步骤。
3. 适用场景：一些方法通用，却在每一个子类都重新写了这一方法。
4. **补充**：模板⽅法与策略模式的不同
   * 模板⽅法是调整流程中部分算法或者流程的处理；策略模式强调替换 完全替换整个算法
   * 模板⽅法基于继承机制： 它允许你通过扩展⼦类中的部分内容来改 变部分算法
   * 策略模式基于组合机制： 模板⽅法在类层次上运作， 因此它是静态 的。 策略在对象层次上运作， 因此允许在运⾏时切换⾏为。

### 代码示例：

1. 核心：抽象出顶层的一系列抽象方法作为算法骨架，并按照逻辑，不改变算法的结构地执行某些特定步骤。
...
1. 在访问者模式（Visitor Pattern）中，我们使用了一个访问者类，它改变了元素类的执行算法。通过这种方式，元素的执行算法可以随着**访问者改变而改变**。
2. **适用场景**： 
...
2. 这种模式实现了一个表达式接口，该接口解释一个特定的上下文。
3. **适用场景**：这种模式被用在 SQL 解析、符号处理引擎等。
